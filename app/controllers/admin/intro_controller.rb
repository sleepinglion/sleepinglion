...
class Admin::IntroController < Admin::AdminController
  ...
  def index
    ...
    @admin_user=User.where(:admin=>true,:intro=>true).first
    @admin_user_images=UserImage.where(:user_id=>@admin_user).page(params[:page]).order('id desc').per(15)
    
    if(params[:id])
      @admin_user_image = UserImage.find(params[:id])
    else
      @admin_user_image=@admin_user_images[0]
    end
