class BlogsController < ApplicationController
  ...
  def index
      if @menu_setting.use_category
        ...
        if(params[:blog_category_id])
          ...
          @blogs = Blog.where(:blog_category_id=>@blog_category_id).order(@menu_setting.order).page(params[:page]).per(@menu_setting.per)          
        else
          @blogs = Blog.order(@menu_setting.order).page(params[:page]).per(@menu_setting.per)        
        end
      else
        @blogs = Blog.order(@menu_setting.order).page(params[:page]).per(@menu_setting.per) 
      end
  ...
  def blog_params
    params.require(:blog).permit(:blog_category_id, :user_id, :title, :description, :tag_list, :photo, :photo_cache, blog_content_attributes: [:id,:content])
  end
end